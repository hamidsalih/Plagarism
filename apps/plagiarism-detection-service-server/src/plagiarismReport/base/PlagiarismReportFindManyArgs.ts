/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PlagiarismReportWhereInput } from "./PlagiarismReportWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { PlagiarismReportOrderByInput } from "./PlagiarismReportOrderByInput";

@ArgsType()
class PlagiarismReportFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => PlagiarismReportWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => PlagiarismReportWhereInput, { nullable: true })
  @Type(() => PlagiarismReportWhereInput)
  where?: PlagiarismReportWhereInput;

  @ApiProperty({
    required: false,
    type: [PlagiarismReportOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [PlagiarismReportOrderByInput], { nullable: true })
  @Type(() => PlagiarismReportOrderByInput)
  orderBy?: Array<PlagiarismReportOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { PlagiarismReportFindManyArgs as PlagiarismReportFindManyArgs };
