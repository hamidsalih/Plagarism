/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { SimilarityCheckWhereUniqueInput } from "../../similarityCheck/base/SimilarityCheckWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class SimilarityCheckUpdateManyWithoutDocumentsInput {
  @Field(() => [SimilarityCheckWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [SimilarityCheckWhereUniqueInput],
  })
  connect?: Array<SimilarityCheckWhereUniqueInput>;

  @Field(() => [SimilarityCheckWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [SimilarityCheckWhereUniqueInput],
  })
  disconnect?: Array<SimilarityCheckWhereUniqueInput>;

  @Field(() => [SimilarityCheckWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [SimilarityCheckWhereUniqueInput],
  })
  set?: Array<SimilarityCheckWhereUniqueInput>;
}

export { SimilarityCheckUpdateManyWithoutDocumentsInput as SimilarityCheckUpdateManyWithoutDocumentsInput };
